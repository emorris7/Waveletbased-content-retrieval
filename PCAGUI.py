...
def main():
    ...
    if len(sys.argv) <= 1:
        ...
        data_array, image_names = PCAKD.extract_features(folder)
        stand_scaler, pca_images, reduced_data = PCAKD.pca_database(data_array)
    ...
    def search_click(event):
        ...
        if number.isdigit() and img_name != "":
            ...
            matching_results, time_taken = PCAKD.find_images_kd(stand_scaler=stand_scaler,
                                                                pca_images=pca_images,
                                                                kd_tree=tree,
                                                                image_names=image_names,
                                                                query_image_name=img_name,
                                                                num_matches=int(number))
            time_var.set("Time taken for query: " + "{:.5f}".format(time_taken))
            precision_var.set(
                "Precision for image query: " + str(PCAKD.calc_rp_kd(img_name, matching_results, int(number))))
            # set up graphics
            unpack_search()
    ...
    def final_save_click(event):
        ...
        if filename != "":
            ...
            ProcessImage.save_images(img_name, matching_results, filename + ".png", pca=True)
            view.pack(pady=20)
            search_again.pack()
    ...
    def view_click(event):
        ProcessImage.show_images(img_name, matching_results, pca=True)
